...
class RTPVideoExtractor:
    def __init__(self, input_pcap, output_file=None, codec="h264"):
        ...
        Args:
            ...
            output_file (str): Path to the output video file (default: based on PCAP filename)
        ...
        if output_file is None:
            # Use PCAP filename as base for output
            pcap_basename = os.path.splitext(os.path.basename(input_pcap))[0]
            output_file = f"{pcap_basename}.{self.get_extension()}"
...
def main():
    ...
    parser.add_argument("-o", "--output", help="Output video file (default: based on input PCAP filename)")
